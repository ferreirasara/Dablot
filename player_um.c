...
void player_um(char TAB[25][21], char player1[]) {
	...
	do {
		printtabuleiro(TAB);
		verifica_pecas_proximas_jog1(TAB);
		linha();
		printf("\t%s, digite a primeira coordenada da posicao inicial: ", player1);
		scanf("%d", &coord_x1);
		...
		// converte as coordenadas para os indices da matriz
		switch(TAB[jog_x1][jog_y1]) {
			case 'R':
			case 'p':
			case 'G': peca_certa = true; break;
			default: peca_certa = false; break;
		}
